...
// API: Submit User Streetlight Data with Weather
app.post('/api/submit_data', async (req, res) => {
    try {
        const data = req.body;
        
        if (!data.asset_id || data.power_w === undefined || data.dim === undefined) {
            return res.status(400).json({ 
                error: 'Missing required fields: asset_id, power_w, dim' 
            });
        }
