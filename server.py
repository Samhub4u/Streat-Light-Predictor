...
def generate_weather(hour, day_offset=0):
    ...
    Returns:
        dict with weather, precipitation, temperature
    """
...
def generate_history_for_asset(doc, start_ts, points, step_minutes):
    """Generate 72-hour history with weather features."""
    hist = []
    aid = doc["asset_id"]
    ...
    for i in range(points):
        ...
        power = (watt * (dim/100.0)) * (0.9 + 0.2*random.random()) if state=="on" else 0.2*random.random()
...
def deduplicate_by_street(docs):
    """Keep only the first asset per unique street"""
    seen = set()
    unique = []
    for d in docs:
