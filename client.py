...
def get_assets():
    """Get list of valid streetlight assets"""
    try:
        load_models()
...
def predict():
    ...
    try:
        ...
        # Weather severity (without visibility)
        ...
        X_anomaly = np.array([[
            power_w,
            100 if predicted_state_binary == 1 else 0,
            power_rolling_mean,
            power_rolling_std,
            power_deviation,
            irregular_on,
...
def health():
    return jsonify({
        ...
        'features': 'Weather-aware (NO visibility)'
    })
